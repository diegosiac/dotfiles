...
def start_multiplexer [] {
    if $MULTIPLEXER_ENV_PREFIX not-in ($env | columns) {
        let session_count = (zellij list-sessions | lines | length)
        if $session_count == 0 {
            run-external zellij attach "-c"
        } else {
            let session_name = (random chars --length 8 | str join "")
            run-external zellij "--session" $session_name
        }
    }
}
